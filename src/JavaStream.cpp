...
bool InputStream::loadDataResource(const char* fileName)
{
	return loadDataFile(fileName, LT_RESOURCE);
}
bool InputStream::loadDataFile(const char* fileName, int loadType) {
	char namePath[2048];
	this->cursor = 0;

	if (loadType == LT_RESOURCE) {
		strncpy(namePath, dir, sizeof(namePath));
		strncat(namePath, "/data/", sizeof(namePath));
		strncat(namePath, fileName, sizeof(namePath) - 1);
		//printf("namePath %s\n", namePath);

		this->data = CAppContainer::getInstance()->zipFile->readDataFileEntry(namePath, &this->fileSize);
		if (this->data) {
			return true;
		}
	}
