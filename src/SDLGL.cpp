...
bool SDLGL::Initialize() {
	...
	if (!this->initialized) {
        ...
        #ifdef __aarch64__
		this->winVidWidth = 1280;
		this->winVidHeight = 720;
		#else
		this->winVidWidth = 400;
		this->winVidHeight = 240;
		#endif

		this->window = SDL_CreateWindow("Doom II RPG By [GEC] Version 0.1", 0, 0, this->winVidWidth, winVidHeight, flags);
		if (!this->window) {
