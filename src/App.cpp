...
Image* Applet::loadDataImage(char* fileName, bool isTransparentMask) {
	InputStream iStream = InputStream();
	Image* img;

	if (iStream.loadDataResource(fileName) == 0) {
		Applet::Error("Failed to open image: %s", fileName);
	}
...
void Applet::loadTables() {
	...
	this->resource->loadIntTable((int*)this->combat->tableCombatMasks, 4);
	...
	this->resource->loadIntTable((int*)this->render->sinTable, 9);
	...
	this->resource->loadIntTable((int*)this->canvas->movieEffects, 12);
